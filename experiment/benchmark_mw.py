...
def run(args):
    ...
    max_replans = 5
    #probe_dir = "/home/wuroderi/metaworld_tasks/ACTIVATIONS"

    #classifier_path = "/home/wuroderi/metaworld_tasks/classifier_final/checkpoints/model_best_mlp.pt"
    classifier_path = "/home/wuroderi/metaworld_tasks/classifier_final/checkpoints/model_best.pt"

    #classifier_model = classifier.simple_classifier(x_input_dim=640, hidden_dim=1024, output_dim=2)
    #classifier_model = classifier.VideoClassifierTransformer(
        #in_channels=640, 
        #num_frames=7, 
        #height=8, 
        #width=8, 
        ...
        #num_layers=2, 
        #num_classes=2)
    classifier_model = classifier.VideoFrameBinaryClassifier(in_channels=640, height=8, width=8, num_classes=2)
    #classifier_model = torch.nn.DataParallel(classifier_model)
    classifier_model.load_state_dict(torch.load(classifier_path))
    #classifier_model.eval()

    video_model = get_video_model(ckpts_dir=args.ckpt_dir, milestone=args.milestone, probe_dir=None)
    ...
    for camera in cameras:
        ...
        for seed in tqdm(range(n_exps)): 
            try: 
                ...
                os.makedirs(f'{result_root}/g{args.gw}/activations/{env_name}/{camera}_{seed}', exist_ok=True) 
                video_model.model.fail_classifier = classifier_model
                video_model.model.fail_classifier.to(video_model.device)
                video_model.model.guidance_weight = args.gw
                video_model.model.save_path = f"{result_root}/g{args.gw}/activations/{env_name}/{camera}_{seed}"
                video_model.ema.ema_model.fail_classifier = classifier_model
                video_model.ema.ema_model.fail_classifier.to(video_model.device)
                video_model.ema.ema_model.guidance_weight = args.gw
                video_model.ema.ema_model.save_path = f"{result_root}/g{args.gw}/activations/{env_name}/{camera}_{seed}"
    ...
    with open(f"{result_root}/result_dict_g{args.gw}.json", "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gw", type=float, default=1.0)
    args = parser.parse_args()

    try:
        with open(f"{args.result_root}/result_dict_g{args.gw}.json", "r") as f:
            result_dict = json.load(f)
    except:
        result_dict = {}
    #result_dict = {}
