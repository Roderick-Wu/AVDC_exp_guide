...
class GoalGaussianDiffusion(nn.Module):
    ...
    def p_sample_loop(self, shape, x_cond, task_embed, return_all_timesteps=False):
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            ...
            # if t % 10 == 9:
            #     self.probe.save_activations("end_input_blocks", f"activations_{t:03d}.npy", use_numpy=True)
        ...
        ax2 = ax1.twinx()
        ax2.set_ylabel("Classifier Score", color='tab:red')
        ax2.plot(self.recorded_data["classifier_scores"], label='Classifier Score', color='tab:red')
...
class Trainer(object):
    ...
    def load(self, milestone):
        ...
        self.ema.ema_model.fail_classifier = self.classifier
        self.model.fail_classifier = self.classifier
    ...
    def sample(self, x_conds, tasks):
        ...
        with self.accelerator.autocast():
            self.ema.ema_model.probe.probe_dir = self.model.probe.probe_dir
