...
class GoalGaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        # self.model_encoder = unet.UNetEncoder(self.model)
        # self.fail_classifier = classifier

        self.probe = probing.prober(probe_dir)
    ...
    def p_sample(self, x, t: int, x_cond, task_embed, guidance_weight=0, target_labels=None):
        ...
        if target_labels is not None and guidance_weight > 0.0:
            ...
            loss = F.cross_entropy(logits[0], target_labels)
        ...
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x, batched_times, x_cond, task_embed, clip_denoised = True, guidance_weight=0)
    ...
    #@torch.no_grad()
    def p_sample_loop(self, shape, x_cond, task_embed, return_all_timesteps=False, guidance_weight=0, target_labels=None):
        ...
        self.model.unet.input_blocks[14][0].out_layers[3].register_forward_hook(self.probe.get_activations_with_grad("end_input_blocks"))

        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            # self_cond = x_start if self.self_condition else None
            img, x_start = self.p_sample(img, t, x_cond, task_embed, guidance_weight=guidance_weight, target_labels=target_labels)
            imgs.append(img)
    ...
    def sample(self, x_cond, task_embed, batch_size = 16, return_all_timesteps = False, guidance_weight=0):
        ...
        return sample_fn((batch_size, channels, image_size[0], image_size[1]), x_cond, task_embed,  return_all_timesteps = return_all_timesteps, guidance_weight=guidance_weight, target_labels=torch.tensor([[1, 0] for _ in range(7)], dtype=torch.float).to(x_cond.device))
...
class Trainer(object):
    ...
    def load(self, milestone):
        ...
        # model.model_encoder = unet.UNetEncoder(model.model)
        # model.fail_classifier = self.classifier
        # self.model.load_additional_models(self.classifier)

        print(self.ema.__class__.__name__)
        print(self.ema.ema_model.__class__.__name__)
        print(id(self.ema))
        print(id(self.ema.ema_model))

        #self.ema.ema_model.create_encoder_wrapper()
        self.ema.ema_model.load_classifier(self.classifier)
        
        #self.model.create_encoder_wrapper()
        self.model.load_classifier(self.classifier)

        print("Loaded additional models")
    ...
    def sample(self, x_conds, batch_text, batch_size=1, guidance_weight=0):
        ...
        print("EMA Model ID in sample():", id(self.ema.ema_model))
