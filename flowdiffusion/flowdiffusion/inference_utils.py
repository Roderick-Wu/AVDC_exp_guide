...
def get_video_model(ckpts_dir='../ckpts/metaworld', milestone=24, flow=False, timestep=100, probe_dir=None):
...
def pred_video(model, frame_0, task, flow=False):
    ...
    output = model.sample(image.to(device), text).cpu() ##
    preds = rearrange(output.squeeze(0), "(f c) w h -> f c w h", c=channels) #
    if not flow:
    ...
    target_size = (128, 128)
    #import pdb; pdb.set_trace()
    target_size = model.model.image_size
    #output = output[0].reshape(-1, 3, *target_size)
    #output = torch.cat([image.unsqueeze(0), output], dim=0)
    output_gif = model.model.save_path + '/metaworld_sample.gif'
    output = (preds.numpy().transpose(0, 2, 3, 1).clip(0, 1) * 255).astype('uint8')
    imageio.mimsave(output_gif, output, duration=1000, loop=1000)
    ###
